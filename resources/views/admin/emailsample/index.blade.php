            ...
            @slot('body')
              @forelse ($samples as $sample)
                <tr id="emailsample-{{ $sample->id }}">
                  ...
                  <td>{!!  nl2br($sample->text) !!}</td>
                  <td>{{ $sample->creator->full_name ?? "-" }}</td>
                  
                  <td>
                    <div class="btn-group">
                      <a class="edit btn btn-warning btn-xs" href="{{ route("admin.emailsample.edit", $sample->id) }}">
                        <i class="fa fa-pencil"></i>
                      </a>
                      <a class="delete btn btn-danger btn-xs"
                        delete-id="{{ $sample->id }}" delete-name="{{ $sample->name }}" href="javascript:;">
              ...
              @empty
                <tr>
                  <td colspan="6">E-posta örneği bulunmamaktadır.</td>
                </tr>
              @endforelse
            @endslot
          @endcomponent
